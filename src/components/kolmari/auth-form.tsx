...
export function AuthForm({ mode, nextPath = '/dashboard' }: { mode: 'login' | 'signup'; nextPath?: string }) {
  ...
  return (
    <form onSubmit={submit} className="mt-8 space-y-5">
      ...
      <label className="block text-sm font-bold text-navy">Password<span className="relative mt-2 block"><input className="field pr-12" name="password" type={showPassword ? 'text' : 'password'} minLength={signup ? 10 : 8} maxLength={72} autoComplete={signup ? 'new-password' : 'current-password'} required placeholder={signup ? 'At least 10 characters, letters and numbers' : 'Your password'} /><button type="button" onClick={() => setShowPassword((value) => !value)} className="absolute inset-y-0 right-0 grid w-12 place-items-center text-muted" aria-label={showPassword ? 'Hide password' : 'Show password'}>{showPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button></span></label>
      ...
      <p className="text-center text-sm text-muted">{signup ? 'Already planning with Kolmari?' : 'New to Kolmari?'} <Link href={authHref(signup ? 'login' : 'signup', nextPath)} className="font-extrabold text-gold-deep">{signup ? 'Sign in' : 'Create an account'}</Link></p>
    </form>
  )
}
