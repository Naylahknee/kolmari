...
function BillingPanel({ paid }: { paid:boolean }) { return <div className="card-surface p-6 sm:p-8"><div className="flex items-start gap-3"><span className="grid size-9 place-items-center rounded-[var(--radius-field)] bg-gold-soft"><CreditCard size={16} className="text-gold-deep"/></span><div><h2 className="font-bold text-navy">Your plan</h2><p className="mt-0.5 text-sm text-muted">Manage your Kolmari subscription.</p></div></div><div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-[var(--radius-field)] border border-line bg-canvas p-4"><div className="flex items-center gap-2">{paid?<CheckCircle2 size={18} className="text-ok"/>:<Sparkles size={18} className="text-gold-deep"/>}<span className="font-semibold text-navy">{paid?'Kolmari Plus — active':'Free plan'}</span></div>{!paid&&<Link href="/coming-soon?feature=plus" className="gold-button">Build My Kolmari Plan <ArrowRight size={15}/></Link>}</div><p className="mt-4 text-sm leading-6 text-muted">Kolmari is launching free. Paid plans and payment management are coming soon — nothing is charged today.</p></div> }
