...
import { SaveNextinationButton } from '@/components/kolmari/saved-nextinations'
...
export default async function CountryDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div>
      <Link href="/countries" className="inline-flex items-center gap-2 text-sm font-bold text-muted"><ArrowLeft size={16} />All Destinations</Link>
      <section className="relative mt-5 overflow-hidden rounded-[22px] bg-navy-deep p-7 text-white sm:p-10">
        ...
        <div className="relative flex flex-wrap items-end justify-between gap-6">
          <div><span className="text-6xl" aria-hidden>{countryFlag(country.code)}</span><p className="mt-5 flex items-center gap-2 text-sm text-gold"><MapPin size={15} />{country.city} · {country.region}</p><h1 className="mt-2 font-display text-5xl font-bold">Make {country.name} your destination.</h1><p className="mt-4 max-w-2xl leading-7 text-white/70">{country.summary}</p></div>
          <div className="max-w-xs rounded-card border border-white/15 bg-white/10 p-5 backdrop-blur"><strong className="block text-gold">{complete ? 'Kolmari Profile connected' : 'Personalization not started'}</strong><span className="mt-1 block text-xs text-white/65">{complete ? 'Use Pathways to compare this country against your facts.' : 'Complete your profile to see personalized matches.'}</span></div>
        </div>
      </section>
      <div className="mt-6 grid gap-5 lg:grid-cols-[1.25fr_.75fr]">
        ...
        <aside className="card-surface h-fit p-6"><p className="text-sm font-bold text-gold-deep">Ready to compare?</p><h2 className="mt-2 font-display text-3xl font-bold">Make this destination personal.</h2><p className="mt-3 text-sm leading-6 text-muted">{complete ? 'Review official requirements against your saved Kolmari Profile.' : 'Complete your Kolmari Profile without any assumed income, work, or household details.'}</p><Link href={complete ? '/pathways' : '/profile-wizard'} className="gold-button mt-6 w-full">{complete ? 'Open Pathways' : 'Start Profile Wizard'} <ArrowRight size={16} /></Link><Link href="/cost-calculator" className="mt-3 inline-flex min-h-12 w-full items-center justify-center rounded-xl border border-line font-bold">Research costs</Link><SaveNextinationButton slug={country.slug} className="mt-3 w-full" /><PassportIndexLink countrySlug={country.slug} countryName={country.name} /></aside>
      </div>
    </div>
  )
}
